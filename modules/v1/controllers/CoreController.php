...
class CoreController extends Controller
{
    ...
    public function actionNodeProcessing($schedule_id = null): bool
    {
        ...
        try {
            ...
            foreach ($nodes as $node) {
                ...
                $config_file = $dir_path . DIRECTORY_SEPARATOR . "{$node['id']}($date).txt";
    ...
    private function getDeviceVersionsLimit() {
        $config = Config::findOne(['key' => 'device_versions']);
        return $config ? (int) $config->value : 5; 
    }
    ...
    public function actionSetWorkerResult(): array
    {
        ...
        switch($data['put']) {
            ...
            case 'file':
                ...
                $node = Node::getNode($data['nodeId']);
                

                /** @var OutputProcessing $this->module->output */
                /** @noinspection PhpUndefinedFieldInspection */
                $storagePath = $this->module->output->getFullPath($data['dataPath'], $data['taskName'], $node['hostname']);
                ...
                $date = Yii::$app->formatter->asDatetime('now', 'php:'.Setting::get('datetime'));
                $f_name  = $storagePath . DIRECTORY_SEPARATOR . $node['ip'] . '_' . $date . '.txt';
                set_error_handler(
                ...
                $this->cleanOldVersions($storagePath);
                ...
                //     if(!$outputModel->delete()) {
                //         Yii::$app->response->statusCode = 500;
                //         return ApiHelper::getResponseBodyByCode(500, 'Can not delete the record from the database.');
                // }
    ...
    private function cleanOldVersions($storagePath) {
        $limit = $this->getDeviceVersionsLimit();
        $files = glob($storagePath . DIRECTORY_SEPARATOR . '*.txt');
